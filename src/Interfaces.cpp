...
void Interfaces::create()
{
	try
	{
		...
		for(std::map<std::string, Systems::PPhysicalInterfaceSettings>::iterator i = _physicalInterfaceSettings.begin(); i != _physicalInterfaceSettings.end(); ++i)
		{
			...
			GD::out.printDebug("Debug: Creating physical device. Type defined in insteon.conf is: " + i->second->type);
			if(i->second->type == "insteonhubx10") device.reset(new InsteonHubX10(i->second));
		...
		if(!GD::defaultPhysicalInterface) GD::defaultPhysicalInterface = std::make_shared<IInsteonInterface>(std::make_shared<BaseLib::Systems::PhysicalInterfaceSettings>());
	}
