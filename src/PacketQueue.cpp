	...
	try
	{
		...
		if(destinationAddress != 0 || force) //Resend when no response?
		{
			...
			try
			{
				...
				GD::bl->threadManager.join(_resendThread);
				_stopResendThread = false;
				GD::bl->threadManager.start(_resendThread, true, GD::bl->settings.packetQueueThreadPriority(), GD::bl->settings.packetQueueThreadPolicy(), &PacketQueue::resend, this, _resendThreadId++);
			}
			catch(const std::exception& ex)
			{
				GD::out.printEx(__FILE__, __LINE__, __PRETTY_FUNCTION__, ex.what());
	...
	try
	{
		...
		if(_queue.empty()) {
			...
			else
			{
				...
				GD::bl->threadManager.join(_pushPendingQueueThread);
				GD::bl->threadManager.start(_pushPendingQueueThread, true, GD::bl->settings.packetQueueThreadPriority(), GD::bl->settings.packetQueueThreadPolicy(), &PacketQueue::pushPendingQueue, this, sendImmediately);
				_pushPendingQueueThreadMutex.unlock();
				return;
			}
		}
