...
//RPC Methods
PVariable InsteonPeer::getDeviceInfo(BaseLib::PRpcClientInfo clientInfo, std::map<std::string, bool> fields)
{
	try
	{
...
PVariable InsteonPeer::setInterface(BaseLib::PRpcClientInfo clientInfo, std::string interfaceID)
{
	try
	{
...
PVariable InsteonPeer::setValue(BaseLib::PRpcClientInfo clientInfo, uint32_t channel, std::string valueKey, PVariable value, bool wait)
{
	try
	{
		...
		for(std::vector<std::shared_ptr<Parameter::Packet>>::iterator i = setRequests.begin(); i != setRequests.end(); ++i)
		{
			...
			{
				queue->push(central->getMessages()->find(frame->responseType, frame->responseSubtype, InsteonPacketFlags::DirectAck, std::vector<std::pair<uint32_t, int32_t>>()));
			}
			else
			{
